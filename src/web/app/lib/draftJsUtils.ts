...
export function selectionHasMutuallyExclusiveStyles(
  editorState: EditorState,
  mutuallyExclusiveStyles: string[]
): boolean {
  if (!mutuallyExclusiveStyles.length || !editorState)
    return false;
  ...
  let inlineKey, foundStyles: string[] = [];
    ...
    for (let i = 0; i < selectedBlocks.size; i += 1) {
      ...
      for (let j = blockStart; j < blockEnd; j += 1) {
        ...
        if (inlineStylesAtOffset) {
          ...
          if (mutuallyExclusiveStyles.includes(inlineKey) && !foundStyles.includes(inlineKey)) {
            foundStyles.push(inlineKey);
...
export function toggleMutuallyExclusivenlineStyle(
  toggleStyle: string,
  editorState: EditorState,
  mutuallyExclusiveustomStyleMap: { [style: string]: { [key: string]: any } },
  ...
  if (!selectionState.getHasFocus()) {
    ...
    newEditorState = EditorState.forceSelection(newEditorState, restoreFocusSelection);
  }

  // Selecting text -> Make sure any style is overlapping
  if (!selectionState.isCollapsed()) {
    Object.keys(mutuallyExclusiveustomStyleMap).forEach(style => {
    ...
    const activeStyle = getMutuallyExclusiveInlineStyle(newEditorState, mutuallyExclusiveustomStyleMap);
    if (activeStyle)
...
export function toggleMutuallyExclusiveCustomInlineStyle(
  ...
  if (!selectionState.isCollapsed()) {
    ...
    const activeStyle = getMutuallyExclusiveInlineStyle(newEditorState, mutuallyExclusiveCustomStyleMap);
    if (activeStyle)
      newEditorState = RichUtils.toggleInlineStyle(newEditorState, activeStyle);
  }
...
export function getMutuallyExclusiveInlineStyle(
  editorState: EditorState,
  mutuallyExclusiveStyleMap: { [style: string]: { [key: string]: any } }
): string | null {
  return editorState ? editorState.getCurrentInlineStyle().findKey((v, k) => {
    return Object.keys(mutuallyExclusiveStyleMap).some(s => s === k);
  }) : null;
}